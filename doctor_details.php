...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['fname'], $_POST['lname'], $_POST['phone'], $_POST['dob'], $_POST['gender'], $_POST['address'])) {
    ...
    // Insertion of data
    $stmt = $con->prepare('INSERT INTO `doctor_details`(`userid`, `firstname`, `lastname`, `phone`, `dob`, `gender`, `address`) VALUES (?,?,?,?,?,?,?);');
    $stmt->bind_param('issssss', $userid, $_POST['fname'], $_POST['lname'], $_POST['phone'], $_POST['dob'], $_POST['gender'], $_POST['address']);
    $stmt->execute();

    echo '<div class="alert alert-success" role="alert">Successfully Completed..<a href="welcome_doctor.php">Go to Welcome Page</a></div>';
    $stmt->close();
    $con->close();
